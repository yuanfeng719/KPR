...
def adjust_relativeAF_basedOn_tag(genoDic,tagDic):
        ...
        for contig in contigLst:
                ...
                if 'DLA64' in genoDic[contig] and 'DLA79' in genoDic[contig]:
        ...
        for contig in contigLst:
                info = genoDic[contig].split('\t')
                if 'DLA64' not in genoDic[contig] and 'DLA79' not in genoDic[contig]:
        ...
        for contig in contigLst:
                if 'DLA64' not in contig and 'DLA79' not in contig:
...
if __name__ == '__main__':
	...
	resultPath = sys.argv[2]
        ...
        DLA79nucSeq = sys.argv[4]
	DLA88_depthCutoff = int(sys.argv[5]) 
        DLA12_depthCutoff = int(sys.argv[6])
        lengthCutoff = int(sys.argv[7]) # 270
	DLAsam = sys.argv[8]
	...
	for i in range(len(lst)):
		...
		contig = contigName
		if 'DLA64' not in lst[i] and 'DLA79' not in lst[i]:
	...
	if DLA64 != []:
        ...
        if DLA79 != []:
                seqDic = seq_dic(DLA79nucSeq)
                for contig in DLA79:
                        geno = genoDic[contig]
                        seq = seqDic[contig.split('NormDLA79')[1]]
                        origRAF = origPerc[contig]
                        allele = geno.split('\t')[3]
                        if allele != '':
                                tag = 'Known allele'
